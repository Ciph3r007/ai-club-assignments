...
`tool_registry.schemas()` and `tool_registry.all()` at build time, so adding
a tool to the registry automatically wires it into the graph.

To add a new tool, register it before calling `create_graph`:
...
class ToolRegistration:
    ...
    Attributes:
        name: Must match the @tool("name") string in the LangChain schema.
        schema: LangChain StructuredTool instance bound to the model.
        handler: ToolHandler instance invoked when the LLM calls this tool.
        node_name: Name of the LangGraph node that will execute this tool.
        has_retry: When True, the graph wires a conditional edge from this node
            to sql_repair_node on execution error. Default False.
    """
...
class ToolRegistry:
    ...
    def handler(self, name: str) -> ToolHandler:
        """Return the handler for the named tool.

        Args:
            name: Registered tool name.

        Returns:
            ToolHandler: The handler instance for this tool.

        Raises:
            ConfigurationError: If the tool is not registered.
        """
    ...
    def get(self, name: str) -> ToolRegistration:
        """Return the full ToolRegistration for the named tool.

        Args:
            name: Registered tool name.

        Returns:
            ToolRegistration: The full registration record.
